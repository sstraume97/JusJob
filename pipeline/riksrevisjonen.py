"""Henter rapporter og dokumenter fra Riksrevisjonen.

Riksrevisjonen publiserer:
  - Forvaltningsrevisjoner (Dokument 3:X)
  - Regnskapsrevisjoner
  - Selskapskontroll

Scraper:
  https://www.riksrevisjonen.no/rapporter-mappe/

Rettsområde: Stats-/statsforfatnings-/statsborgerrett / Forvaltnings-/kommunalrett
Output: data/riksrevisjonen.jsonl.gz
"""
...
BASE = "https://www.riksrevisjonen.no"
USER_AGENT = "JusJob-DataPipeline/0.1 (+https://github.com/sstraume97/JusJob)"
DELAY = 1.2

LIST_URLS = [
    f"{BASE}/rapporter-mappe/",
    f"{BASE}/rapporter/",
]

SUBJECTS = ["Stats-/statsforfatnings-/statsborgerrett", "Forvaltnings-/kommunalrett"]

DATA_DIR = Path(__file__).resolve().parent.parent / "data"
OUT_PATH = DATA_DIR / "riksrevisjonen.jsonl.gz"
CACHE_FILE = DATA_DIR / "riksrevisjonen_cache.json"
...
def _extract_doc_number(text: str) -> str:
    m = re.search(r"Dokument\s+\d+:\d+\s*\(\d{4}[–-]\d{4}\)|Dok\.\s*\d+:\d+", text, re.I)
    ...
    while url and page < 100:
        ...
        for a in soup.find_all("a", href=True):
            ...
            if full.rstrip("/") == list_url.rstrip("/"):
                continue
            if not re.search(r"/rapporter|/\d{4}[/-]|dokument-\d", full, re.I):
                continue
            seen.add(full)
            found += 1
            parent_text = a.parent.get_text(" ", strip=True) if a.parent else ""
            date_m = re.search(r"\d{1,2}\.\d{1,2}\.\d{4}|\d{4}-\d{2}-\d{2}|\d{4}[–-]\d{4}", parent_text)
            doc_number = _extract_doc_number(text + " " + parent_text)
            items.append({
                "url": full, "title": text,
...
def main() -> None:
    ...
    for list_url in LIST_URLS:
        print(f"\n📋 Riksrevisjonen — {list_url}", flush=True)
        items = _scrape_list(session, list_url)
        if not items:
            continue
        print(f"  {len(items)} rapporter funnet", flush=True)
        new_count = 0
        for item in items:
            url = item["url"]
            ...
            doc_type = "forvaltningsrevisjon" if re.search(r"Dok|3:", item.get("doc_number", "")) else "rapport"
            all_new.append(Document(
                url=url, doc_type=doc_type, title=item["title"],
                date=date or item["date"], doc_number=item["doc_number"],
