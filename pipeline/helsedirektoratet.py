"""Henter rundskriv, veiledere og lovfortolkninger fra Helsedirektoratet.

Scraper:
  https://www.helsedirektoratet.no/rundskriv
  https://www.helsedirektoratet.no/veiledere
  https://www.helsedirektoratet.no/tolkninger

Rettsområde: Helse- og omsorgsrett
Output: data/helsedirektoratet.jsonl.gz
"""
...
BASE = "https://www.helsedirektoratet.no"

PAGES = [
    ("rundskriv",   f"{BASE}/rundskriv",   "rundskriv"),
    ("veileder",    f"{BASE}/veiledere",   "veileder"),
    ("tolkning",    f"{BASE}/tolkninger",  "lovfortolkning"),
]

SUBJECTS = ["Helse-/omsorgsrett"]

OUT_PATH = DATA_DIR / "helsedirektoratet.jsonl.gz"
CACHE_FILE = DATA_DIR / "helsedirektoratet_cache.json"
    ...
    while url and page < 100:
        ...
        for a in soup.find_all("a", href=True):
            ...
            if full.rstrip("/") == list_url.rstrip("/"):
                continue
            if not re.search(r"/rundskriv/|/veiledere/|/tolkninger/|/\d{4}[/-]", full, re.I):
                continue
            seen.add(full)
            found += 1
