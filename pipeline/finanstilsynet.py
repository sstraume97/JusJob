"""Henter vedtak og tillatelser fra Finanstilsynet.

Scraper:
  https://www.finanstilsynet.no/nyheter/?type=vedtak
  https://www.finanstilsynet.no/nyheter/?type=tillatelser
  https://www.finanstilsynet.no/nyheter/?type=brev-og-uttalelser

Rettsområde: Bank/finans/regnskapsrett
Output: data/finanstilsynet.jsonl.gz
"""
...
BASE = "https://www.finanstilsynet.no"
USER_AGENT = "JusJob-DataPipeline/0.1 (+https://github.com/sstraume97/JusJob)"
DELAY = 1.2

PAGES = [
    ("vedtak",     f"{BASE}/nyheter/?type=vedtak",               "vedtak"),
    ("tillatelse", f"{BASE}/nyheter/?type=tillatelser",          "tillatelse"),
    ("uttalelse",  f"{BASE}/nyheter/?type=brev-og-uttalelser",   "brev/uttalelse"),
    ("rundskriv",  f"{BASE}/nyheter/?type=rundskriv",            "rundskriv"),
]

SUBJECTS = ["Bank/finans/regnskapsrett"]

DATA_DIR = Path(__file__).resolve().parent.parent / "data"
OUT_PATH = DATA_DIR / "finanstilsynet.jsonl.gz"
CACHE_FILE = DATA_DIR / "finanstilsynet_cache.json"
    ...
    while url and page < 150:
        ...
        for a in soup.find_all("a", href=True):
            ...
            if full.rstrip("/") == list_url.rstrip("/"):
                continue
            # Finanstilsynet-nyheter/vedtak har typisk /nyheter/YYYY/... URL
            if not re.search(r"/nyheter/\d{4}|/vedtak/\d|/tillatelse|/brev", full, re.I):
                continue
            seen.add(full)
            found += 1
        ...
        # Paginering: ?side=N eller ?page=N
        neste = soup.find("a", string=re.compile(r"neste|next|›|»", re.I))
        if not neste:
            neste = soup.find("a", href=re.compile(r"[?&](side|page)=\d+"))
