"""Henter vedtak fra Skatteklagenemnda.

Scraper:
  https://www.skatteklagenemnda.no/vedtak/

Vedtak publiseres med vedtaksreferanse (f.eks. SKN-2024-01).
Rettsområde: Skatte-/avgiftsrett
Output: data/skatteklagenemnda.jsonl.gz
"""
...
BASE = "https://www.skatteklagenemnda.no"
...
LIST_URLS = [
    f"{BASE}/vedtak/",
    f"{BASE}/avgjorelser/",
    f"{BASE}/nyheter/vedtak/",
]

SUBJECTS = ["Skatte-/avgiftsrett"]

DATA_DIR = Path(__file__).resolve().parent.parent / "data"
OUT_PATH = DATA_DIR / "skatteklagenemnda.jsonl.gz"
CACHE_FILE = DATA_DIR / "skatteklagenemnda_cache.json"
...
def _extract_case_ref(text: str, url: str) -> str:
    m = re.search(r"SKN[- ]\d{4}[- ]\d+|SKNS[- ]\d|NS[- ]\d+/\d{4}", text + " " + url, re.I)
    if m:
    ...
    while url and page < 100:
        ...
        for a in soup.find_all("a", href=True):
            ...
            if full.rstrip("/") == list_url.rstrip("/"):
                continue
            if not re.search(r"/\d{4}[/-]|/vedtak/|/avgjor|skn", full, re.I):
                continue
            seen.add(full)
            found += 1
            parent_text = a.parent.get_text(" ", strip=True) if a.parent else ""
            date_m = re.search(r"\d{1,2}\.\d{1,2}\.\d{4}|\d{4}-\d{2}-\d{2}", parent_text)
            case_ref = _extract_case_ref(text + " " + parent_text, full)
            items.append({
                "url": full, "title": text,
...
def main() -> None:
    ...
    for list_url in LIST_URLS:
        print(f"\n📋 Skatteklagenemnda — {list_url}", flush=True)
        ...
        for item in items:
            ...
            all_new.append(Document(
                ...
                date=date or item["date"], case_ref=item["case_ref"],
