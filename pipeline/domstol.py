"""Henter fritt tilgjengelige avgjørelser fra Norges domstoler (domstol.no).

Scraper:
  https://www.domstol.no/no/domsavgjorelser/
  https://www.domstol.no/enkelt-domstol/sivile-saker/avgjorelser/

Merk: Domstolene har ingen bulk-API. Fulltekst-dommer distribueres via
Lovdata/rettspraksis.no — domstol.no publiserer utvalgte avgjørelser og
sammendrag (særlig prinsipielle saker).
Rettsområde: Alle (settes ikke statisk).
Output: data/domstol.jsonl.gz
"""
...
BASE = "https://www.domstol.no"
USER_AGENT = "JusJob-DataPipeline/0.1 (+https://github.com/sstraume97/JusJob)"
DELAY = 1.2

LIST_URLS = [
    f"{BASE}/no/domsavgjorelser/",
    f"{BASE}/domsavgjorelser/",
    f"{BASE}/no/aktuelt/nyheter/",
]

DATA_DIR = Path(__file__).resolve().parent.parent / "data"
OUT_PATH = DATA_DIR / "domstol.jsonl.gz"
CACHE_FILE = DATA_DIR / "domstol_cache.json"
...
def _extract_case_number(text: str) -> str:
    m = re.search(r"\b\d{2}-\d{6}[A-Z]{3}-[A-ZÆØÅ/]+\b|HR-\d{4}-\d+-[A-Z]", text)
    ...
    while url and page < 100:
        ...
        for a in soup.find_all("a", href=True):
            ...
            if full.rstrip("/") == list_url.rstrip("/"):
                continue
            if not re.search(r"/domsavgjor|/avgjorelser/|/\d{4}[/-]|HR-\d{4}", full, re.I):
                continue
            seen.add(full)
            found += 1
            parent_text = a.parent.get_text(" ", strip=True) if a.parent else ""
            date_m = re.search(r"\d{1,2}\.\d{1,2}\.\d{4}|\d{4}-\d{2}-\d{2}", parent_text)
            court_m = re.search(r"(Høyesterett|lagmannsrett|tingrett|Arbeidsretten)", text + " " + parent_text, re.I)
            items.append({
                "url": full, "title": text,
...
def _fetch_detail(session: requests.Session, url: str) -> tuple[str, str, str]:
    ...
    court = ""
    court_m = re.search(r"(Norges Høyesterett|Høyesterett|\w+ lagmannsrett|\w+ tingrett|Arbeidsretten)",
                        soup.get_text(" ", strip=True))
    if court_m:
        court = court_m.group(0)
    snippet = ""
...
def main() -> None:
    ...
    for list_url in LIST_URLS:
        print(f"\n📋 Domstol.no — {list_url}", flush=True)
        ...
        for item in items:
            ...
            if url in cache or url in existing:
                continue
            snippet, date, court = _fetch_detail(session, url)
            all_new.append(Document(
                url=url, doc_type="avgjørelse", title=item["title"],
                date=date or item["date"], court=court or item["court"],
                case_number=item["case_number"], snippet=snippet,
            ))
            new_count += 1
